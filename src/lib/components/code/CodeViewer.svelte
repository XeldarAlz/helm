...
<div class="flex flex-col h-full">
  ...
  <div class="flex-1 overflow-auto bg-[var(--color-bg-deep)]">
    <div class="flex min-w-fit">
      ...
      <!-- Code content -->
      <pre class="flex-1 p-0 m-0"><code class="block text-[12px] font-mono leading-[1.6] text-[var(--color-text-primary)]">{#each lines as line, i}<div class="px-4 hover:bg-[var(--color-bg-surface)]/30">{line || " "}</div>{/each}</code></pre>
    </div>
  </div>
</div>
